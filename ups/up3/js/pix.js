...
/* =============================================================================
 * PIX Panel — Vanilla JS
 * Chama SEMPRE o backend proprio do projeto:
 *   POST /api/create-transaction   (gera a cobranca Pix via Velani)
 *   GET  /api/transaction-status   (consulta status, usado no polling)
 * A Secret Key da Velani fica SOMENTE no servidor (pasta /api) - nunca
 * neste arquivo nem em nenhum outro codigo que roda no navegador.
 * ========================================================================== */

/* Garante que o payload sempre tenha nome/email/telefone/cpf validos.
   Prioriza os dados REAIS do comprador (vindos do checkout, via localStorage
   funil_nome/funil_email/funil_phone/funil_cpf). So preenche telefone/CPF
   automaticamente se, por algum motivo, o dado real nao estiver disponivel -
   nunca inventamos nome/e-mail. */
function sanitizeClient(c) {
  ...
  out.phone = (ph.length === 10 || ph.length === 11) ? ph : '';
  var doc = String(c.document || '').replace(/\D/g, '');
  out.document = doc.length === 11 ? doc : '';
  return out;
}
...
function initPixPanel(containerId, config) {
  /*
    config: {
      productType, productName, amount,
  ...
  */
  ...
  function handlePixResponse(d) {
    if (!d || d.success === false || !d.pix_code || !d.transaction_id) {
      var errMsg = (d && d.error) || 'Não foi possível gerar o Pix. Tente novamente.';
      container.innerHTML = buildError(errMsg, accent);
      return;
    }
    txId = d.transaction_id;
    var pixQr = resolveQrSource(d.pix_qrcode_image || null);
    container.innerHTML = buildReady(d.pix_code, pixQr, config.amount, accent);
    mountQr(container, d.pix_code, pixQr);
    bindCopyButton(d.pix_code, accent);
    startPixCountdown();
    startPolling();
  }
  ...
  fetch('/api/create-transaction', {
    ...
    body: JSON.stringify({
      ...
      comprador: {
        nome: (config.client && config.client.name) || '',
        email: (config.client && config.client.email) || '',
        telefone: (config.client && config.client.phone) || '',
        cpf: (config.client && config.client.document) || ''
      },
      carrinho: [{
        titulo: config.productName || 'Produto',
        preco: config.amount,
        quantidade: 1
      }],
      utm: getUtmString()
  ...
  function startPolling() {
    ...
    function tick() {
      ...
      var delay = document.hidden ? 6000 : (fast ? 2000 : 4000);
  ...
  function checkStatus() {
    ...
    fetch('/api/transaction-status?id=' + encodeURIComponent(txId))
      .then(function (r) { return r.json().catch(function () { return null; }); })
      .then(function (d) {
        if (!d || d.success === false) return;
        var norm = String(d.status || '').toUpperCase();
        if (norm === 'PAID' || norm === 'APPROVED') handlePaid();
      })
      .catch(function () {});
  }
...
/* Resolve a origem da imagem do QR Code (URL, data-uri ou base64 puro). */
...
/* ===== QR Code client-side =====
   A Velani normalmente devolve so o codigo Pix copia-e-cola, sem imagem do
   QR. Por isso o QR e sempre gerado no proprio navegador a partir do codigo,
   usando a biblioteca embutida no topo deste arquivo (window.qrcode). */
...
function generateQrDataUrl(text) {
  ...
  for (var i = 0; i < levels.length; i++) {
    try {
      var qr = window.qrcode(0, levels[i]);
